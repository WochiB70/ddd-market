...
public class QueryCurrencyController {

    private final JPAQueryFactory queryFactory;

    @GetMapping("/query/currencies")
    @Operation(summary = "查询货币列表", description = "获取所有货币的基本信息")
    @ApiResponse(responseCode = "200", description = "成功返回货币列表",
            content = @Content(mediaType = "application/json",
                    schema = @Schema(implementation = QueryCurrencyResponse.class)))
    public List<QueryCurrencyResponse> queryCurrencies() {
