...
public record CountLimit(
    ...
    public CountLimit {
        Objects.requireNonNull(identifierId, "活动ID不能为空");
    ...
    public void participate(UserId userId) {
        ...
        switch (type) {
            ...
            case DAY_COUNT -> {
                if (count >= activityRepository().countParticipateActivityByUserIdInDay(identifierId, userId)) {
                    throw new ActivityLimitException("今天参与活动次数已达上限，请明天在参与活动！");
                }
            }
            case THIS_WEEK_COUNT -> {
                if (count >= activityRepository().countParticipateActivityByUserIdInThisWeek(identifierId, userId)) {
                    throw new ActivityLimitException("本周参与活动次数已达上限，请下周在参与活动！");
                }
            }
            case RECENTLY_WEEK_COUNT -> {
                if (count >= activityRepository().countParticipateActivityByUserIdInRecentlyWeek(identifierId, userId)) {
                    throw new ActivityLimitException("最近一周参与活动次数已达上限，请下周在参与活动！");
                }
            }
            case THIS_MONTH_COUNT -> {
                if (count >= activityRepository().countParticipateActivityByUserIdInThisMonth(identifierId, userId)) {
                    throw new ActivityLimitException("本月参与活动次数已达上限，请下月在参与活动！");
                }
            }
            case RECENTLY_MONTH_COUNT -> {
                if (count >= activityRepository().countParticipateActivityByUserIdInRecentlyMonth(identifierId, userId)) {
                    throw new ActivityLimitException("最近一个月参与活动次数已达上限，请下月在参与活动！");
                }
            }
            case YEAR_COUNT -> {
                if (count >= activityRepository().countParticipateActivityByUserIdInYear(identifierId, userId)) {
                    throw new ActivityLimitException("本年参与活动次数已达上限，请明年在参与活动！");
                }
            }
            case ACTIVITY_COUNT -> {
                if (count >= activityRepository().countParticipateActivityByUserIdInAllTime(identifierId, userId)) {
